    ...
    try {
        ...
        for (let mbi = 0; mbi < monkeyBlockInstructions.length; mbi++) {
            ...
            const test = testArray.map(t => {
                ...
                const numberString = splittedTest[splittedTest.length - 1];
        ...
        const superModulo = monkeys.reduce((acc, value) => acc * value.test[0], 1);
        for (let r = 0; r < rounds; r++) {
            for (let m = 0; m < monkeys.length; m++) {
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    const worryLevel = getWorryLevel(operator, inspectedItem, operand)%superModulo;
                    
                    if (worryLevel%divisibilityOperand === 0) monkeys[successMonkey].items.push(worryLevel);
                    else monkeys[failMonkey].items.push(worryLevel)
        ...
        const monkeyBusiness = monkeys
            .sort((ma, mb) => mb.inspectTimes - ma.inspectTimes)
            .slice(0, 2)
